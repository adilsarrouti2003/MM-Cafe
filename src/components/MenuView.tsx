    ...
    <div className="py-24 sm:py-36 bg-[#FAF7F2] text-[#1C1C1C] relative animate-fade-in">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-24">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-4">
          ...
          <p className="text-gray-550 text-xs sm:text-sm font-light leading-relaxed max-w-md mx-auto font-sans">
            {d.menuSubtitle}
          </p>
          <div className="w-12 h-[1px] bg-[#B89020]/45 mx-auto pt-2" />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
          {filteredItems.map((item: MenuItem) => {
            return (
              <div
                key={item.id}
                className="group relative flex flex-col sm:flex-row gap-6 p-6 bg-white border border-[#B89020]/15 hover:border-[#B89020]/35 transition-all duration-500 shadow-[0_15px_40px_rgba(184,144,32,0.02)] rounded-3xl hover-premium-lift"
              >
                {/* Visual Frame */}
                <div className="w-full sm:w-32 h-32 aspect-square relative overflow-hidden bg-[#FAF7F2] border border-[#B89020]/10 shrink-0 rounded-2xl shadow-sm">
                  <img
                    src={item.image}
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <button
                      ...
                      className="p-2 bg-black/90 text-[#D4AF37] border border-[#D4AF37]/30 hover:bg-[#D4AF37] hover:text-black transition-all cursor-pointer"
                    >
                  ...
                  {item.popular && (
                    <div className="absolute top-2 left-2 bg-[#B89020] text-white px-2 py-0.5 text-[7px] font-mono tracking-widest uppercase font-black shadow-md rounded-none">
                      {d.popupPopular}
                    </div>
                  )}
                </div>
                ...
                <div className="flex-grow flex flex-col justify-between">
                  <div className="space-y-2">
                    <div className="flex justify-between items-baseline gap-4">
                      <h3 className="font-serif text-base sm:text-lg text-[#1C1C1C] font-semibold group-hover:text-[#B89020] transition-colors">
                        {language === 'ar' ? item.titleAr : item.titleFr}
                      </h3>
                      
                      <div className="flex flex-col items-end shrink-0">
                        <span className="text-[#B89020] font-mono font-bold text-base sm:text-lg">
                          {item.price}
                        </span>
                    ...
                    <p className="text-gray-500 text-xs sm:text-sm leading-relaxed font-light line-clamp-2 font-sans">
                      {language === 'ar' ? item.descAr : item.descFr}
                    </p>
                  </div>

                  <div className="pt-4 flex items-center justify-between text-[10px] text-gray-400 font-mono border-t border-[#B89020]/10 mt-2">
                    <span className="flex items-center gap-1">
                      <Utensils className="w-3 h-3 text-[#B89020]/60" />
                    ...
                    <button
                      ...
                      className="text-[#B89020] hover:text-[#1C1C1C] uppercase tracking-wider font-bold cursor-pointer transition-colors duration-250"
                    >
        ...
        {/* 2. ADVANCED STREAK: REAL QR-MENU ACCESS CARD */}
        <div className="bg-white border border-[#B89020]/15 p-8 sm:p-12 relative overflow-hidden rounded-3xl shadow-[0_15px_45px_rgba(184,144,32,0.02)]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="flex items-center gap-2">
                <div className="p-2 bg-[#B89020]/10 text-[#B89020] border border-[#B89020]/20 rounded-lg">
                  <QrCode className="w-5 h-5" />
                </div>
                <span className="text-xs font-mono text-[#B89020] uppercase tracking-widest block font-black">
                  {isRtl ? 'الخدمة الذاتية السريعة QR CODE' : 'Contactless Table Service Hub'}
                </span>
              </div>
              
              <h3 className="text-2xl sm:text-3xl font-serif text-[#1C1C1C] leading-snug">
                {isRtl 
                  ? 'اطلب مباشرة من طاولتك عبر ميزة الـ QR المدمجة' 
                  : 'Scan and Enjoy Premium Table-Side Self Checkout'}
              </h3>
              
              <p className="text-gray-500 text-xs sm:text-sm leading-relaxed max-w-2xl font-sans font-light">
                {isRtl 
            ...
            <div className="lg:col-span-4 flex justify-center">
              <div className="p-6 bg-[#FCFAF5] text-black text-center space-y-2 select-none border border-[#B89020]/30 rounded-2xl shadow-lg">
                <div className="w-28 h-28 mx-auto bg-neutral-900 flex items-center justify-center p-2 rounded-xl shadow-inner">
                  <span className="text-white text-3xl animate-bounce">☕</span>
                </div>
                <span className="text-[10px] font-mono font-black uppercase text-gray-500 tracking-wider">Shams Al Madina</span>
                <p className="text-xs font-bold text-gray-900 border-t border-[#B89020]/15 pt-2">{isRtl ? 'مسح رمز المقهى' : 'Scan to View Menu'}</p>
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-[#1C1B19] to-[#0E0D0C] text-white border border-[#B89020]/25 p-8 sm:p-16 text-center space-y-6 relative overflow-hidden rounded-3xl shadow-2xl">
          <div className="absolute top-0 right-0 w-64 h-64 bg-[#B89020]/5 rounded-bl-full pointer-events-none" />
          <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#B89020]/3 rounded-tr-full pointer-events-none" />
          
          <div className="max-w-2xl mx-auto space-y-6 relative z-10">
            <span className="text-xs font-mono uppercase text-[#D4AF37] tracking-[0.2em] font-black block">
            ...
            {loyaltySubscribed ? (
              <motion.div 
                ...
                className="p-6 bg-[#FCFAF5] border border-[#B89020] text-center space-y-3 rounded-xl shadow-lg animate-fade-in"
              >
                ...
                <h4 className="font-serif text-[#1C1C1C] font-semibold text-base">
                  {isRtl ? 'مبارك! تم تفعيل بطاقة VIP الخاصة بك' : 'Welcome to the VIP Circle'}
                </h4>
                <p className="text-[#B89020] font-mono text-xs uppercase tracking-widest font-black">
              ...
              <form onSubmit={handleLoyaltySubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
                <input
                  ...
                  className="flex-grow bg-white border border-[#B89020]/20 focus:border-[#D4AF37] outline-none px-4 py-3.5 text-xs sm:text-sm text-black rounded-none transition-all font-sans"
                />
                
                <button
                  type="submit"
                  className="px-6 py-3.5 bg-[#D4AF37] text-black font-black uppercase tracking-widest text-xs rounded-none hover:bg-white cursor-pointer transition-all border border-[#D4AF37]"
                >
            ...
            <p className="text-[10px] text-gray-550 font-mono tracking-wider font-light">
              {isRtl 
      ...
      {selectedImage && (
        ...
        >
          <div className="max-w-4xl relative border border-white/10 shadow-2xl bg-[#0A0A0A]">
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute top-4 right-4 z-10 p-2 bg-black/80 text-white hover:text-[#D4AF37] border border-white/10 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            <img 
